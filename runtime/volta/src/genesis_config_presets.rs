...
pub fn volta_testnet_config_genesis() -> Result<serde_json::Value, sp_core::crypto::PublicError> {
...
pub fn volta_local_config_genesis() -> serde_json::Value {
...
pub fn volta_development_config_genesis() -> serde_json::Value {
    let balances = DEFAULT_ENDOWED_SEEDS
        .into_iter()
        .map(|seed| {
            FundedAccount::from_id(
...
pub fn get_preset(id: &sp_genesis_builder::PresetId) -> Option<Vec<u8>> {
    let cfg = match id.as_ref() {
        "development" => volta_development_config_genesis(),
        "local" => volta_local_config_genesis(),
        "testnet" => volta_testnet_config_genesis().unwrap(),
        _ => return None,
    };
    Some(
