...
pub fn determine_workers_paths(
    ...
    if !prep_worker_path.is_executable() || !exec_worker_path.is_executable() {
        return Err(Error::InvalidWorkerBinaries {
            prep_worker_path,
            exec_worker_path,
        });
    }
...
pub const PREPARE_BINARY_NAME: &str = "zkv-relay-prepare-worker";

/// The name of binary spawned to execute a PVF
pub const EXECUTE_BINARY_NAME: &str = "zkv-relay-execute-worker";
...
fn build_worker_paths(
    ...
    let (prep_worker_name, exec_worker_name) = workers_names.unwrap_or((
        PREPARE_BINARY_NAME.to_string(),
        EXECUTE_BINARY_NAME.to_string(),
    ));
