...
impl<'a> DummyCrate<'a> {
    ...
    fn prepare_command(&self, subcommand: &str) -> Command {
        ...
        if !self.ignore_target {
            cmd.arg(format!(
                "--target={}",
                self.target.rustc_target(self.cargo_command)
            ));
        }
    ...
    pub(crate) fn is_target_installed(&self, target: &str) -> Option<bool> {
        let sysroot = self.get_sysroot()?;
        let target_libdir_path = Path::new(sysroot.trim())
            .join("lib")
            .join("rustlib")
            .join(target)
            .join("lib");
    ...
    fn try_build(&self) -> Result<(), Option<String>> {
        let Ok(result) = self.prepare_command("build").output() else {
            return Err(None);
        };
